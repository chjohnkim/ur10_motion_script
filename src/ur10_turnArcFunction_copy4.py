...
def TurnArcAboutAxis(group, axis, CenterOfCircle_1, CenterOfCircle_2, angle_degree, direction, change_orientation, ee_axis, ee_direction):
    ...
    while theta < angle_degree/180.0 * math.pi:
        ...
        if change_orientation is 'yes':      
           ...
           # euler = quaternion_to_euler(quaternion[0], quaternion[1], quaternion[2], quaternion[3])     
            ...
            if ee_axis is 'ee_x' :
                roll += ee_direction*math.pi/tolerance
            if ee_axis is 'ee_y' :
                pitch += ee_direction*math.pi/tolerance
            if ee_axis is 'ee_z' :
                yaw += ee_direction*math.pi/tolerance
       ...
       #print waypoints
        theta+=math.pi/tolerance # increment counter, defines the number of waypoints 
    del waypoints[:2]
...
def manipulator_arm_control():
    ...
    TurnArcAboutAxis(group, 'y', 0, 0, 45, 1, 'no', 'ee_z', -1)
...
#    TurnArcAboutAxis(group, 'z', 0.7, 0, 90, -1, 'yes', 'ee_z', -1)
#    TurnArcAboutAxis(group, 'y', 0.4, 0.7, 90, 1, 'yes', 'ee_y', 1)
